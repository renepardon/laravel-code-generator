...
namespace Renepardon\CodeGenerator\Commands;

use App;
use Renepardon\CodeGenerator\Commands\Bases\ControllerCommandBase;
...
class CreateControllerCommand extends ControllerCommandBase
{
    ...
    public function getNameInput()
    {
        ...
        return Helpers::convertSlashToBackslash($path . Helpers::postFixWith($name,
                Config::getControllerNamePostFix()));
    }
    ...
    public function handle()
    {
        ...
        $viewVariablesForIndex = $this->getCompactVariablesFor($fields, $this->getPluralVariable($input->modelName),
            'index');
        $viewVariablesForShow = $this->getCompactVariablesFor($fields, $this->getSingularVariable($input->modelName),
            'show');
        $viewVariablesForEdit = $this->getCompactVariablesFor($fields, $this->getSingularVariable($input->modelName),
            'form');
        ...
        $namespacesToUse = $this->getRequiredUseClasses($fields,
            [$modelNamespace, $requestNameSpace, $classToExtendFullname]);
    ...
    protected function getCommandInput()
    {
        ...
        $controllerName = $cName ? str_finish($cName,
    ...
    protected function getAffirmMethod($withFormRequest, array $fields, $requestNamespace, $input)
    {
        ...
        $this->replaceValidationRules($stub, $this->getValidationRules($fields))
            ->replaceFileValidationSnippet($stub,
                $this->getFileValidationSnippet($fields, $input, $this->requestVariable))
            ->replaceRequestFullName($stub, $requestNamespace);
    ...
    {
        return $this->getStubContent('controller-getdata-method-5.5');
    }
