...
class StandardHtml extends HtmlGeneratorBase
{
    ...
    protected function getFieldPlaceHolderForMenu(Label $placeholder = null, $name = '')
    {
        ...
        return sprintf('<option value="" style="display: none;" {{ %s == \'\' ? \'selected\' : \'\' }} disabled selected>%s</option>',
            $this->getRawOptionValue($name, ''), $this->getTitle($placeholder, true));
    }
    ...
    {
        return sprintf(" {{ %s ? 'selected' : '' }}",
            $this->getMultipleRawOptionValue($name, $valueAccessor, $defaultValue));
    }
    ...
    {
        return sprintf(" {{ %s == %s ? 'selected' : '' }}", $this->getRawOptionValue($name, $defaultValue),
            $valueAccessor);
    }
