...
namespace Renepardon\CodeGenerator\HtmlGenerators;

use Exception;
use Renepardon\CodeGenerator\Models\Field;
use Renepardon\CodeGenerator\Models\Label;
use Renepardon\CodeGenerator\Support\ValidationParser;
...
abstract class HtmlGeneratorBase
{
     ...
     *
     * @var Renepardon\CodeGenerator\Support\ViewLabelsGenerator
     */
    ...
    protected function getPickItemsHtml(Field $field, ValidationParser $parser)
    {
        ...
        $this->wrapField($fields, $field, true,
            (($field->htmlType == 'checkbox') && ! $field->isBoolean()) ? 'required' : '');
    ...
    protected function getPickItemsHtmlField(Field $field, Label $option, ValidationParser $parser)
    {
        ...
        $fieldName = ($field->isMultipleAnswers()) ? $this->getFieldNameAsArray($field->name) : $field->name;
        $checkedItem = $this->getCheckedItemForPickItem($option->value, $field->name, $field->isMultipleAnswers(),
            $field->htmlValue);
    ...
    {
        return $isMultiple ? $this->getMultipleCheckedItem($value, $name, $defaultValue) : $this->getCheckedItem($value,
            $name, $defaultValue);
    }
