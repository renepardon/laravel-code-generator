...
class FieldOptimizer
{
    ...
    protected function optimizeValidations()
    {
        ...
        if (($rule = $this->field->getDateValidationRule()) != null && ! in_array($rule,
    ...
    {
        if (! array_key_exists('is-nullable',
                $this->meta) && ! $this->field->isPrimary() && ($this->parser->isNullable() || ! $this->parser->isRequired() || $this->parser->isConditionalRequired())) {
    ...
    {
        if (empty($this->field->methodParams) && in_array($this->field->dataType,
                ['string', 'char']) && ! empty($this->parser->getMaxLength())) {
