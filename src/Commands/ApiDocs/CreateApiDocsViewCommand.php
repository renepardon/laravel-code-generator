...
namespace Renepardon\CodeGenerator\Commands\ApiDocs;

use Illuminate\Console\Command;
use Renepardon\CodeGenerator\Models\Field;
use Renepardon\CodeGenerator\Models\Label;
...
use Renepardon\CodeGenerator\Support\ViewLabelsGenerator;
use Renepardon\CodeGenerator\Traits\ApiDocViewsTrait;
use Renepardon\CodeGenerator\Traits\ApiResourceTrait;
use Renepardon\CodeGenerator\Traits\CommonCommand;
use Renepardon\CodeGenerator\Traits\GeneratorReplacers;
use Renepardon\CodeGenerator\Traits\LanguageTrait;
use Renepardon\CodeGenerator\Traits\RouteTrait;
...
class CreateApiDocsViewCommand extends Command
{
    ...
    public function handle()
    {
        ...
        return $this->replaceAuthorizedRequestForIndex($stub,
            $this->getAuthorizedRequestForIndex($input->withAuth, $resource->getApiDocLabels(),
                $viewLabels->getLabels()))
            ->replaceAuthorizationCall($stub,
                $this->getAuthorizationCall($input->withAuth, $resource->getApiDocLabels(), $viewLabels->getLabels()))
            ->replaceFailedAuthorizationCall($stub, $this->getFailedAuthorizationCall($input->withAuth))
            ->replaceFieldsListForBody($stub,
                $this->getFieldsListBody($input->modelName, $resource, $viewLabels->getLabels(), true))
            ...
            ->replaceRouteNames($stub, $input->modelName,
                $this->getNamePrefix($input->prefix, 'api', $input->apiVersion))
    ...
    protected function getCommandInput()
    {
        ...
        $controllerName = $cName ? Str::finish($cName,
            Config::getControllerNamePostFix()) : Helpers::makeControllerName($modelName);
    ...
    protected function makeFieldsListView($input, Resource $resource, array $standardLabels = null)
    {
        ...
        $this->replaceAuthorizationCall($stub,
            $this->getAuthorizationCall($input->withAuth, $resource->getApiDocLabels(), $standardLabels))
            ->replaceStandardLabels($stub, $standardLabels)
            ->replaceApiLabels($stub, $resource->getApiDocLabels())
            ->replaceModelName($stub, $input->modelName)
            ->replaceValidationRuleRequired($stub,
                $this->getRequiredRule($resource->getApiDocLabels(), $standardLabels))
     ...
     *
     * @param  Renepardon\CodeGenerator\Model\Label $label
     *
     * @return string
     */
    ...
    protected function getFieldsListBody(
        $modelName,
        Resource $resource,
        array $standardLabels = null,
