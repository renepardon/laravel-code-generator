...
use Renepardon\CodeGenerator\Models\Index;
...
class MysqlParser extends ParserBase
{
    ...
    protected function getRelations()
    {
        ...
        foreach ($rawRelations as $rawRelation) {
            $relations[] = $this->getRealtion($rawRelation->foreignTable, $rawRelation->foreignKey,
                $rawRelation->localKey, $rawRelation->selfReferences);
        }
    ...
    protected function getRealtion($foreignTableName, $foreignColumn, $localColumn, $selfReferences)
    {
        ...
        if ($this->isOneToMany($foreignTableName, $foreignColumn)) {
            return new ForeignRelationship('hasMany', $params,
                Str::camel($relationName . Str::plural($foreignTableName)));
        }
    ...
    protected function getTransfredFields(array $columns)
    {
        ...
        foreach ($columns as $column) {
            ...
            $properties['data-type-params'] = $this->getPrecision($column->CHARACTER_MAXIMUM_LENGTH, $column->DATA_TYPE,
                $column->COLUMN_TYPE);
