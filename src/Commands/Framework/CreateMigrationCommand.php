...
namespace Renepardon\CodeGenerator\Commands\Framework;

use Exception;
use File;
use Renepardon\CodeGenerator\Commands\Bases\MigrationCommandBase;
use Renepardon\CodeGenerator\Models\Field;
use Renepardon\CodeGenerator\Models\ForeignConstraint;
use Renepardon\CodeGenerator\Models\Label;
use Renepardon\CodeGenerator\Models\MigrationCapsule;
use Renepardon\CodeGenerator\Models\MigrationChangeCapsule;
use Renepardon\CodeGenerator\Models\MigrationInput;
use Renepardon\CodeGenerator\Models\MigrationTrackerCapsule;
...
use Renepardon\CodeGenerator\Support\MigrationHistoryTracker;
...
class CreateMigrationCommand extends MigrationCommandBase
{
    ...
    public function handle()
    {
        ...
        if (is_null($migration)) {
            ...
            if ($migration->isMigrated() || $migration->isVirtual) {
                ...
                if ($delta->hasChange()) {
                    ...
                    $newMigration->className = $this->makeAlterTableClassName($this->tableName,
                        $capsule->totalMigrations());
     ...
     *
     * @param array of Renepardon\CodeGenerator\Models\Field $field
     *
     * @return array of Renepardon\CodeGenerator\Models\ForeignConstraint $field
     */
        ...
        foreach ($indexes as $index) {
            ...
            if ($index->hasMultipleColumns()) {
                ...
                if (count($invalidColumns) > 0) {
                    throw new Exception('Non-Existing columns are being using an index. Invalid columns are "' . implode(',',
                            $invalidColumns) . '"');
                }
    ...
    protected function addFieldType(&$property, Field $field)
    {
        ...
        $property .= sprintf("%s('%s'%s)", $this->getPropertyBase($field->getEloquentDataMethod()), $field->name,
            $params);
     ...
     * @return void
     */
    protected function makeAlterMigration(
        MigrationInput $input,
        MigrationCapsule $migration,
        MigrationChangeCapsule $changeCapsule
    ) {
        $stub = $this->getStubContent('migration', $input->template);
    ...
    protected function getSchemaUpAlterBlueprint(MigrationInput $input, MigrationChangeCapsule $changeCapsule)
    {
        ...
        foreach ($fieldsWithChanges as $fieldsWithChange) {
            if ($fieldsWithChange->isDeleted) {
                ...
                $this->addRenameColumn($properties, $fieldsWithChange->fromField->name,
                    $fieldsWithChange->toField->name)
                    ->addFieldPropertyClousure($properties);
            } else {
