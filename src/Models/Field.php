...
class Field implements JsonWriter
{
    ...
    public function setValidationProperty(array $properties)
    {
        if (Arr::isKeyExists($properties, 'validation')) {
            $this->validationRules = is_array($properties['validation']) ? $properties['validation'] : Arr::removeEmptyItems(explode('|',
                $properties['validation']));
        }

        if ($this->isNullable && ! in_array('nullable', $this->validationRules)) {
        ...
        if (in_array($this->getEloquentDataMethod(), ['char', 'string']) && in_array($this->htmlType,
                ['text', 'textarea'])) {
    ...
    protected function setForeignRelationFromArray(array $properties)
    {
        ...
        if (Arr::isKeyExists($properties, 'foreign-relation')) {
            ...
            if (Arr::isKeyExists($properties, 'foreign-constraint')) {
    ...
    public function getAccessorValue($variable, $view = null)
    {
        ...
        if ($this->isBoolean()) {
            return sprintf("(%s) ? '%s' : '%s'", $fieldAccessor, $this->getTrueBooleanOption()->text,
                $this->getFalseBooleanOption()->text);
        }
