...
class CreateModelCommand extends Command
{
    ...
    public function handle()
    {
        ...
        return $this->replaceTable($stub, $resource->getTableName($input->table))
            ...
            ->replaceTimestamps($stub,
                $this->getTimeStampsStub($input->useTimeStamps, $resource->isCreateAndUpdateAtManaged()))
    ...
    protected function replaceSoftDelete(&$stub, $shouldUseSoftDelete)
    {
        if ($shouldUseSoftDelete) {
            $stub = $this->strReplace('use_soft_delete',
                PHP_EOL . 'use Illuminate\Database\Eloquent\SoftDeletes;' . PHP_EOL, $stub);
